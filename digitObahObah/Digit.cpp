...
Digit::Digit(Bitmap* d, DMD3* f, byte value, int pixel, uint16_t xo, uint16_t yo, uint16_t color) {
  _display = d;
  _value = value;
  xOffset = xo;
  yOffset = yo;
...
void Digit::swap(){
 _disp->DMD3::swapBuffers();
}

void Digit::swapcopy(){
 _disp->DMD3::swapBuffersAndCopy();
}
...
{
  _display->setPixel(xOffset + x, height - (y + yOffset), c);
  //swap();
}
...
{
  _display->drawFilledRect(xOffset + x, height - (y + yOffset), w,h, c);
  //(int x1, int y1, int x2, int y2, Color color = White);
 //   swap();
}
...
void Digit::Morph1() {
 ...
 {
	if (i < segWidth) {	
	drawLine(1+i, segHeight * 2 + 2, segWidth, segHeight * 2 + 2, _color);
	drawLine(1+i, 0, segWidth, 0, _color);
	}  
    ...
    drawPixel( i, segHeight * 2 + 2, black); // A
    drawPixel( i, 0, black); // D
    drawPixel( i, segHeight + 1, black); // G
...
void Digit::Morph7() {
  ...
  for (int i = 0; i <= (segWidth+1); i++)
  {
	...
	drawLine(1+i, segHeight + 1, segWidth, segHeight + 1, _color);   //G
	drawLine(1+i, 0, segWidth, 0, _color);
	}  
    // Move E left to right
    drawLine(0 + i - 1, 1, 0 + i - 1, segHeight, black);
